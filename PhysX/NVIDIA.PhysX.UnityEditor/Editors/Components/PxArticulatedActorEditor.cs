...
namespace NVIDIA.PhysX.UnityEditor
{
    ...
    public class PxArticulatedActorEditor : PxActorEditor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(rootInfo, new GUIContent("Root Link"));
            if (rootInfo.isExpanded)
            {
                BeginRecreateCheck();
                EditorGUI.indentLevel++;

                EditorGUILayout.PropertyField(rootInfo.FindPropertyRelative("collisionShape"));

                //EditorGUILayout.Separator();

                var massInfo = rootInfo.FindPropertyRelative("massInfo");
                EditorGUILayout.PropertyField(massInfo, new GUIContent("Mass"));
            ...
            EditorGUILayout.PropertyField(m_linkInfos, new GUIContent("Child Links"));
            if (m_linkInfos.isExpanded)
            {
                ...
                if (m_listIndex > 0 && m_listIndex < m_linkInfos.arraySize)
                {
                    ...
                    EditorGUILayout.PropertyField(jointInfo, new GUIContent("Joint"));
                    if (jointInfo.isExpanded)
                    {
                        ...
                        EditorGUILayout.PropertyField(axisInfos, new GUIContent("Axes"));
                        if (axisInfos.isExpanded)
                        {
                            ...
                            Func<int, string, int> drawAxis = (axisIndex, axisName) =>
                            {
                                ...
                                EditorGUILayout.PropertyField(axisInfo, new GUIContent(axisName));
