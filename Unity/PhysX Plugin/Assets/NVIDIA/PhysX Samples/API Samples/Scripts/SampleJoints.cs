...
public class SampleJoints : SampleBase
{
    #region Messages
    ...
    void CreateObjects()
    {
        ...
        for (int y = 0; y < 10; ++y)
            for (int x = 0; x < 10; ++x)
                CreateBoxChain(Vector3.right * 2.0f * (x - 5) + Vector3.forward * 2.0f * (y - 5) + Vector3.up * 15, 15);
    }

    void CreateBoxChain(Vector3 pos, int length)
    {
        for (int i = 0; i < length; ++i)
        {
            // Create PxRigidDynamic
            var body = physics.createRigidDynamic(new PxTransform((pos + Vector3.up * (length - i)).ToPxVec3()));
            // Add box shape
            ...
            var corners = new PxVec3[] { new PxVec3(0.5f, 0.5f, 0.5f), new PxVec3(-0.5f, 0.5f, -0.5f) };
            var anchor = (pos + Vector3.up * (length - i)).ToPxVec3() + corners[i % corners.Length];
            var body0 = body;
            var transform0 = new PxTransform(body0.getGlobalPose().transformInv(anchor));
            var body1 = i > 0 ? m_dynamicActors[m_dynamicActors.Count - 2] : null;
            var transform1 = new PxTransform(body1 != null ? body1.getGlobalPose().transformInv(anchor) : anchor);

            // Create PxJoint
            var joint = physics.createSphericalJoint(body0, transform0, body1, transform1);
            // Set joint properties
            joint.setConstraintFlag(PxConstraintFlag.COLLISION_ENABLED, true);
            joint.setConstraintFlag(PxConstraintFlag.PROJECTION, true);
        }
    }
